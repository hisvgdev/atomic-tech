...
export async function handleGetHeaderPages() {
  return await fetchData('header/get-all', 'array');
}

export async function handleGetHeaderPage(pageId: string) {
  return await fetchData(`header/get/${pageId}`, 'object');
}